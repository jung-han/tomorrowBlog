...
* Set은 has메소드로 값의 유무를 판정하거나 for-of루프, values프로퍼티로 내부 요소를 열거하는 것 뿐이다.

그리고
* `assign`메소드
    * 기존의 객체를 결합(merge)할 수 있다.
    * 동일 명칭의 프로퍼티는 나중에 정의된 값으로 덮어 쓰인다.
    * 재귀적인 결합은 지원하지 않는다.
    * 만약 pet1, pet2, pet3의 객체를 한 객체로 묶고 싶다면 `let obj = Object.assign({}, pet1, pet2, pet3);`으로 하면 된다.
...
Object.freeze(pet);
```
* 세 메소드 모두 property의 추가는 불가능 
* 프로퍼티의 삭제는(`delete pet.type;`같은 것) seal, freeze불가
* 프로퍼티의 변경은 freeze만 불가

* URL 끝부분의 쿼리 정보에는 몇가지 문자등을 사용할 수 없다.
    * `?, =`같이 미리 사용되고 있거나 `%`, 공백, 멀티바이트 등은 사용할 수 없는데 이러한 것들을 미리 변환 처리하는데 `encodeURI()`메소드를 사용한다.
    * 비슷한 메서드 중에 `escape()`라고 있는데 하위 호환성을 유지하고 싶은 경우 등 특별한 이유가 없다면 사용하지 않는것을 추천한다고 합니다.
* `eval()`메서드는 동적으로 생성한 스크립트를 실행 가능하다.
    * 하지만 시큐리티 리스크나 일반적인 코드의 처리 속도보다 느리므로 사용하지 않는 것을 추천한다.  
## 4장
## 5장
## 6장